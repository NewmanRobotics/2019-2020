...
public class ArmLiftTeleop extends OpMode {
    ...
    public void loop() {
         ...
         *   - [right X] the rotation of the grabber
         *   - [button B] open/close the grabber
         */
        float height = gamepad2.left_stick_y;
        float extension = gamepad2.right_stick_y;
        float rotation = gamepad2.right_stick_x;

        robot.operateGrabber(gamepad2.b ? 1.0 : ArmLiftHardware.ORIGIN);
        robot.armLifter.setPower(height);
        robot.armExtender.setPower(extension);
        robot.rotateGrabber(rotation);
